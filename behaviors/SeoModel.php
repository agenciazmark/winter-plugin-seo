...
class SeoModel extends ModelBehavior
{
    ...
    protected function setProperties($model)
    {
        if (!isset($model->seoPageType)) {
            throw new ApplicationException(trans('zmark.seo::lang.seotab.missing_property', ['model' => class_basename($model)]));
        }
    ...
    protected function extendFields()
    {
        Event::listen('backend.form.extendFields', function ($widget) {
            if (isset($widget->model) && get_class($widget->model) == $this->modelClass && !$widget->isNested) {
                $seoFields = Yaml::parseFile(plugins_path('zmark/seo/models/seo/tab.yaml'));

                $seoFields['zmark_seotab@update']['pageType'] = $this->seoPageType;
