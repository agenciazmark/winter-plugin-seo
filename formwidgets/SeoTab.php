...
{
    protected $defaultAlias = 'zmark_seotab';
    ...
    private function prepareVars()
    {
        ...
        $seo = \Zmark\Seo\Models\Seo::where('type', $this->pageType)
            ->where('reference', $this->model->getKey())->first();

        if ($seo) {
            $this->vars['seoData'] = $seo;
            $this->vars['editSeoUrl'] = Backend::url('zmark/seo/seo/update/'.$seo->id . '?back='.url()->current());
            $this->partial = 'seotab_seodata';
            return;
        }

        $this->vars['editSeoUrl'] = Backend::url('zmark/seo/seo/create?type='.$this->pageType.'&reference='.$this->model->getKey().'&back='.url()->current());

        $currentLocale = \App::getLocale();
        $defaultLocale = \RainLab\Translate\Models\Locale::getDefault()->code;
        $translator = \RainLab\Translate\Classes\Translator::instance();
        $translator->setLocale($defaultLocale, false);
        $seo_defaults_default_locale = Event::fire('zmark.seo.mapSeoData', [$this->pageType, $this->model->getKey()], true);
        ...
        foreach ($alternateLocales as $locale) {
            if ($locale == $defaultLocale) {
                ...
                $seo_defaults = Event::fire('zmark.seo.mapSeoData', [$this->pageType, $this->model->getKey()], true);
            }
            $seoData = new \Zmark\Seo\Models\Seo;
