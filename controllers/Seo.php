...
class Seo extends \Backend\Classes\Controller {
    ...
    public $requiredPermissions = ['zmark.seo.manage'];
	...
	{
	    parent::__construct();
        BackendMenu::setContext('Zmark.Seo', 'seo', 'seo');

        $this->backUrl = Backend::url('zmark/seo/seo');
    ...
    public function import()
    {
        ...
        $this->addJs('/plugins/zmark/seo/assets/js/october.import.js');
    ...
    protected function makeImportUploadFormWidget()
    {
        ...
        $widgetConfig = $this->makeConfig('~/plugins/zmark/seo/controllers/seo/fields_import.yaml');
    ...
    protected function getImportFileColumns()
    {
        ...
        if ($importFileExtension == 'zip') {
            ...
            if (empty($csvPath)) {
                throw new ApplicationException(Lang::get('zmark.seo::lang.seo.zip_doesnt_have_csv_file'));
            }
        } else {
            $csvPath = $path;
        }
